...
code = code.replace(/variations\.forEach\(\(v, idx\) => \{[\s\S]*?grid\.appendChild\(btn\);\s*\}\);/, 
...
// 6. Update clear selection selector
code = code.replace(/document\.querySelectorAll\('\.template-btn, \.canva-tpl-card'\)\.forEach\(b => b\.classList\.remove\('active'\)\);/g, "document.querySelectorAll('.template-btn, .canva-tpl-card').forEach(b => b.classList.remove('active'));");
...
code = code.replace(/    const lsTitle = localStorage\.getItem\('canvaLTitle'\);\s*const lsPrice = localStorage\.getItem\('canvaLPrice'\);\s*const lsFeats = localStorage\.getItem\('canvaLFeats'\);\s*if\(lsTitle\) document\.getElementById\('canvaLTitle'\)\.value = lsTitle;/, lsFix);

fs.writeFileSync(v0Path, code, 'utf8');
console.log("Successfully patched ALL changes directly!");
