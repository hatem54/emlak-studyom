...
const selectCalloutStart = js.indexOf('function selectCalloutEl(el) {');
...
const newSelectCallout = `function selectCalloutEl(el) {
...
`;
if(oldSelectCallout.length > 50) {
    js = js.replace(oldSelectCallout, newSelectCallout);
    fs.writeFileSync('modules/callout.js', js);
    console.log("selectCalloutEl updated successfully.");
} else {
    console.log("Error finding selectCalloutEl.");
}
