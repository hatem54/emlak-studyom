...
// 1. Inject addTextHandles function definition
...
// 2. Patch selectElement to call addTextHandles
...
const targetTabLF = targetTab.replace(/\r\n/g, '\n');
const replacementTab = targetTab + "\n        if (el.classList.contains('canvas-el') && typeof window.addTextHandles === 'function') window.addTextHandles(el);";

if (dragContent.includes(targetTab)) {
    dragContent = dragContent.replace(targetTab, replacementTab);
} else if (dragContent.includes(targetTabLF)) {
    dragContent = dragContent.replace(targetTabLF, replacementTab);
}

// 3. Update the callout check
...
console.log('Successfully re-injected addTextHandles and patched drag.js!');
