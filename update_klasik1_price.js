...
// This puts it inside the photo frame, right at the bottom edge.

let oldBlock = lines.slice(k1Idx, endIdx).join('\n');
let newBlock = oldBlock.replace('top:${scaleY(180)}px', 'bottom:${scaleY(240)}px');

lines.splice(k1Idx, endIdx - k1Idx);
lines.splice(k1Idx, 0, newBlock);

fs.writeFileSync('tpl_klasik/klasik.js', lines.join('\n'));
console.log('Klasik 1 price moved to the bottom of the photo frame.');
